...
class AnomalyDetectionInferenceResponse(CvInferenceResponse):
    """Anomaly Detection inference response.

    Attributes:
        anomaly_score (float): Image-level anomaly score.
        anomaly_map (List): Pixel-level anomaly heatmap as nested list.
    """

    anomaly_score: float = Field(description="Image-level anomaly score")
    anomaly_map: List = Field(description="Pixel-level anomaly heatmap")
